...
template<typename FloatType>
class TruncEuclideanDist_wrapper : public Distance_wrapper<FloatType> {
	int len;
  public:
	TruncEuclideanDist_wrapper(int len) : len(len) {}
	typename Distance_wrapper<FloatType>::DistanceFunction getFunction() override {
		return std::bind(
			Vector<FloatType>::truncEuclidean, 
			std::placeholders::_1,
			std::placeholders::_2,
			len
		);
...
DEFINE_FLOATINSTANTIATIONS(TruncEuclideanDist)
