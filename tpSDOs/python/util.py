...
def lookupDistance(name, float_type, **kwargs):
	...
	elif name == 'trunc_euclidean':
		if not 'len' in kwargs:
			raise TypeError('len is required for truncated euclidean distance')
		return swig.__dict__['TruncEuclideanDist' + suffix](kwargs['len'])
	elif name == 'minkowski':
		if not 'p' in kwargs:
